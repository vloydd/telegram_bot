...
/**
 * Class for specific command which provide search method by city.
 */
class CitySearchCommand extends SystemCommand {
   ...
   * @var string
   */
  protected $description = 'City search';
  ...
  public function execute(): ServerResponse {
    ...
    for ($i = 0; $i < count($entity_list); $i++) {
      ...
      $flight_city[] = $entity_list[$i]->get('city')->getValue()[0]['value'];
      $time = date("H:i", $flight_time[$i]);
      $flight_cities[$entity_list[$i]->get('city')
        ->getValue()[0]['value'] . (' ' . ($i + 10))] = $flight_number[$i] . ' ' . $flight_status[$i] . ' ' . $time;
    }
    ...
    switch ($state) {
      case 0:
        ...
        $text = '';
      case 1:
        if ($text === '' || !in_array($text, $flight_city, TRUE)) {
          $notes['state'] = 1;
          $this->conversation->update();
          $data['text'] = 'Type your city:';
      ...
      case 2:
        $this->conversation->update();
        $city = $notes['city'];
        $data['text'] = 'Sorry, we can`t find your flight, try again :)';
        // Checking for matching user data.
        $inverted_array_cities = array_flip($flight_cities);
        $m_array = preg_grep("/^$city\s.*/", $inverted_array_cities);
        $inverted_array_cities_result = array_flip($m_array);
        $flight_counter = 1;
        // Build answer with array for user.
        foreach ($inverted_array_cities_result as $k => $v) {
          $out_text .= PHP_EOL . $flight_counter++ . '.' . ' ' . substr($k, 0, -2) . ' ' . $v;
        }
        if ($out_text) {
          $data['text'] = $out_text;
        }
